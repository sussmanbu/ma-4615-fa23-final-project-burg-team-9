...
With our overarching objective aimed at generating a comprehensive analysis of traffic accidents to facilitate more equitable allocation and deployment of local police forces by the Montgomery County government, we are steadfast in assigning top priority to the examination of the correlation between race and traffic events. Furthermore we hope to uncover and analyze any potential racial bias, whether intentional or not, in these traffic violations and accidents, specifically through the possible over-representation of traffic events in the most “non-White” areas. This concept not only harmonizes with the thesis statement established at the outset but also underscores the practical significance of race as the most overtly discernible variable. In a holistic sense, delving into the interplay between race and accidents offers local governments measures to inform effective strategies as well as identify and reform any unconscious bias. Building upon this conceptual foundation, we build our primary analysis direction: "What correlations can be identified between Race and Accident?" 
...
library(dplyr)
library(stringr)
crash_data <- read.csv("D:/MA615/Crash_Reporting_-_Incidents_Data.csv")
load("D:/MA615/traffic_violations.RData")
